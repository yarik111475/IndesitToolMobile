...
Evo2EepParser::Evo2EepParser()
{

}

void Evo2EepParser::checkEepData(const QByteArray &eepData)
{
    if(eepData.size() > 0x2000){
    ...
    if(m_eepInfoStartAddress >= eepData.size()-28){
        m_eepModel.IsModelValid=false;
        return;
    }
...
void Evo2EepParser::checkEepChecksum(const QByteArray &eepData)
{
...
void Evo2EepParser::setEepModel(const QByteArray &eepData)
{
    if(!m_eepModel.IsModelValid){
        return;
    }
...
void Evo2EepParser::setEepCode(const QByteArray &eepData)
{
    if(!m_eepModel.IsModelValid){
        return;
    }
...
void Evo2EepParser::setEepSerial(const QByteArray &eepData)
{
    if(!m_eepModel.IsModelValid){
        return;
    }
